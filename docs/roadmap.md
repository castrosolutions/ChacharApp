...
- [ ] **Publish the first release**: the notarized **1.2.1 `.dmg` is live on R2** at
      `https://dl.juanpablocastro.com/releases/1.2.1/ChacharApp-1.2.1.dmg`. First-run fixes found
      ...
      submenu); 1.2.1 fixed push-to-talk (key-handling inversion and an `installTap` crash) —
      the first update delivered in-app over Sparkle. Published at
